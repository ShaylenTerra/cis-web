...
export class ReservationDecisionComponent implements OnInit {
  ...
  submitDecision() {
    ...
    if (this.decisionform.invalid) {
      ...
      decision.assignedToUserId = this.draftData.processId === 239 ? 0 : this.assignUser.userId,
  ...
  addUserNotification() {
    ...
    const notification = {
      ...
      'notifyUserId': this.draftData.processId === 239 ? 0 : this.assignUser.userId,
  ...
  getAllUserByUserType(roleid) {
    ...
    this.restService.getUserByRoleIdProvinceId(this.taskDetail.provinceId, roleid).subscribe(response => {
      ...
      this.loaderService.display(false);
      this.assignDecisionUser = this.draftData.processId === 239 ? 0 : this.users[0].userId;
