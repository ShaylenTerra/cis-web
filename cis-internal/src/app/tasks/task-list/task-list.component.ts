...
import { ConfirmeventDialogComponent } from './confirmevent-dialog/confirmevent-dialog.component';
...
export class TaskListComponent implements OnInit {
    ...
    displayedColumns: string[] = ['processName', 'referenceNo', 'triggeredOn', 'assignedToUser', 'actionRequiredDescription', 'priorityName', 'internalStatusCaption', 'sla', 'action'];
    dataLength: number;
    dataSource: any;
    ...
    filterAll() {
        ...
        if (this.selectedRequestType.length > 0) {
            this.filteredTasks = this.filteredTasks.length > 0 ? this.filteredTasks.filter((task) => {
                return this.selectedRequestType.indexOf(task.processName) !== -1;
        ...
        if (this.selectedAction.length > 0) {
            this.filteredTasks = this.filteredTasks.length > 0 ? this.filteredTasks.filter((task) => {
                return this.selectedAction.indexOf(task.actionRequiredDescription) !== -1;
            }) : this.tasks.filter((task) => {
                return this.selectedAction.indexOf(task.actionRequiredDescription) !== -1;
            });
        }
        ...
        if (this.selectedPriority.length > 0) {
            this.filteredTasks = this.filteredTasks.length > 0 ? this.filteredTasks.filter((task) => {
                return this.selectedPriority.indexOf(task.priorityName) !== -1;
            }) : this.tasks.filter((task) => {
                return this.selectedPriority.indexOf(task.priorityName) !== -1;
            });
        }
    ...
    updateBadges() {
        ...
        this.filteredTasks.forEach(t => {
            ...
            this.priorityList.forEach(pl => {
                if (pl.key === t.priorityName) {
                    pl.count++;
                }
            });
            this.Action.forEach(a => {
                if (a.key === t.actionRequiredDescription) {
                    a.count++;
                }
            });
    ...
    gotoTaskDetails(taskDetail) {
        if (taskDetail.processName === 'Reservation Request' || taskDetail.processName === 'Reservation Transfer') {
    ...
    refreshTable() {
        ...
        this.dataSource.sortingDataAccessor = (item, property) => {
            switch (property) {
                case 'sla': return item.triggeredOn;
    ...
    getInboxTasks(stat: any) {
        ...
        this.restService.getInboxTask(this.userId).subscribe(payload => {
            ...
            this.requestTypes = this.distinct(this.tasks, 'processName');
            this.priorityList = this.distinct(this.tasks, 'priorityName');
            this.Action = this.distinct(this.tasks, 'actionRequiredDescription');
