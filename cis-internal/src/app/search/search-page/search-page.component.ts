...
import { SearchRequestModalComponent } from './search-request-modal/search-request-modal.component';
...
export class SearchPageComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    constructor(private router: Router, private restService: RestcallService,
        ...
        private dialog: MatDialog, private fb: FormBuilder, private topMenu: TopMenuService) {
        if (this.router.getCurrentNavigation() !== null) {
            if (this.router.getCurrentNavigation().extras.state !== undefined) {
                this.filterDataBack = this.router.getCurrentNavigation().extras.state.filterData;
                this.requestorDataBack = this.router.getCurrentNavigation().extras.state.reqData;
            }
        }
        ...
        if (this.requestorDataBack !== undefined) {
            if (this.requestorDataBack.requesterType !== 447) {
                this.requestorClient = this.requestorDataBack.requesterType;
                this.requestor = 'no';
                this.requesterForm.patchValue({
                    firstName: this.requestorDataBack.requesterDetails.firstName,
                    surName: this.requestorDataBack.requesterDetails.surName,
                    email: this.requestorDataBack.requesterDetails.email,
                    fax: this.requestorDataBack.requesterDetails.fax,
                    addressLine1: this.requestorDataBack.requesterDetails.addressLine1,
                    addressLine2: this.requestorDataBack.requesterDetails.addressLine2,
                    addressLine3: this.requestorDataBack.requesterDetails.addressLine3,
                    contactNo: this.requestorDataBack.requesterDetails.contactNo,
                    postalCode: this.requestorDataBack.requesterDetails.postalCode
                });
            } else {
                this.requestorClient = 447;
            }
        }
    ...
    initialise(stat: any) {
        ...
        ]).subscribe(([provinces, searchTypes, reqType]) => {
    ...
    requestSearch() {
        const dialogRef = this.dialog.open(SearchRequestModalComponent, {
            ...
            data: {
                ...
                // requesterForm: this.requesterForm.value,
    ...
    openNgiDetails(ngiDetails) {
        const searchData = {
            ...
            // templateListItemId: this.searchFilter.templateListItemId
            // township: this.Township,
            // municipality: this.municipality
        };
        const data = {
    ...
    getSearchData(stat) {
        this.loaderService.display(true);
        this.assignTownship = '';
        this.restService.getSearchData(this.page, this.size, 'dated,desc', this.userId, '').subscribe(searchData => {
