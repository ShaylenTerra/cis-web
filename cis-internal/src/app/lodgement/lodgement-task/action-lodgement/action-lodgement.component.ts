...
@Component({
  selector: 'app-action-lodgement',
  templateUrl: './action-lodgement.component.html',
  styleUrls: ['./action-lodgement.component.css']
})
export class ActionLodgementomponent implements OnInit {
  form: FormGroup;
  userId = JSON.parse(sessionStorage.getItem('userInfo')).userId;
  uploadedFileName1 = 'Upload document';
  constructor(public dialogRef: MatDialogRef<ActionLodgementomponent>,
  ...
  notifyReservation(val) {
    this.loaderService.display(true);
    const obj = {
      'referenceNumber': val.referenceNo,
      'templateId': val.templateId,
      'transactionId': val.transactionId,
      'context': val.workflowId,
      'workflowId': val.workflowId
    };
    this.restService.notifyForLodgement(obj).subscribe((res: any) => {
      this.loaderService.display(false);
    }, error => {
      this.loaderService.display(false);
    });
