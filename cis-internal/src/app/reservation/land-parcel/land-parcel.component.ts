...
export class LandParcelComponent implements OnInit, OnDestroy, OnChanges {
  ...
  creationfarms = ReservationReason.CREATIONOFFARMS;
  ...
  public filteredMajorRegion: ReplaySubject<any[]> = new ReplaySubject<any[]>(1);
  public majorRegionFilterCtrl: FormControl = new FormControl();
  protected _onDestroyMajorRegion = new Subject<void>();
  isAddStep = false;
  newTownshipName
  constructor(private restService: RestcallService, private fb: FormBuilder,
    ...
    this.creationFarmForm = this.fb.group({
      farmName: ''
    });
    ...
    this.restService.getAllDraftSteps(this.draftId).subscribe(payload => {
      ...
      if (this.resDraftData !== null && this.resDraftData.length > 0) {
        for (let i = 0; i < this.resDraftData.length; i++) {
          ...
          if (this.resDraftData[i].reasonItemId === ReservationReason.CREATIONOFFARMS) {
            this.creationFarmForm.patchValue({
              farmName: this.resDraftData[i].otherData?.toLocationName
            })
          }
        }
      }
  ...
  processOutcome(stepId, reasonItemId) {
    ...
    if (reasonItemId === ReservationReason.CREATIONOFFARMS) {
      if (this.creationFarmForm.invalid) {
        this.creationFarmForm.get('farmName').markAsTouched();
        this.snackbar.openSnackBar('Enter New Farm Name', 'Warning');
        return;
      }
    }
    ...
    if (reasonItemId === ReservationReason.CREATIONOFTOWNALLOTMENTAREASTOWNSHIPS) {
      if (stepObj.reservationDraftRequests.length > 0) {
        stepObj.otherData = { 'toLocationId': 0, 'toLocationName': this.townshipForm.value.newTownshipName }
      }
    }

    if (reasonItemId === ReservationReason.CREATIONOFFARMS) {
      if (stepObj.reservationDraftRequests.length > 0) {
        stepObj.otherData = { 'toLocationId': 0, 'toLocationName': this.creationFarmForm.value.farmName }
      }
    }

    if (reasonItemId === ReservationReason.EXTENSIONOFTOWNSHIPS || reasonItemId === ReservationReason.RELAYOUT) {
      if (stepObj.reservationDraftRequests.length > 0) {
  ...
  landInfo(data) {
    ...
    const dialogRef = this.dialog.open(LandInfoComponent, {
      width: 'auto',
      height: 'auto',
      data: { value: data, header: 'Reservation' }
    });
    dialogRef.afterClosed().subscribe(res => {
    });
  }
  ...
  getMajorRegionOrAdminDistrict() {
    // this.assignTownship = '';
    this.form.get('majorRegion').reset();
    this.majorRegionData = [];
    // this.MajorRegionOrAdminDistrict = null;
    this.restService.getMajorRegionOrAdminDistrict(this.provinceId).subscribe(response => {
      this.majorRegionData = response.data;
      this.filteredMajorRegion.next(this.majorRegionData.slice());
      this.townshipFilterCtrl.valueChanges
        .pipe(takeUntil(this._onDestroyMajorRegion))
        .subscribe(() => {
          this.filterMajorRegion();
        });
    });
  ...
  protected filterMajorRegion() {
    if (!this.majorRegionData) {
      return;
    }
    let search = this.majorRegionFilterCtrl.value;
    if (!search) {
      this.filteredMajorRegion.next(this.majorRegionData.slice());
      return;
    } else {
    ...
    this.filteredMajorRegion.next(
      this.majorRegionData.filter(x => x.majorRegionOrAdminDistrict.toLowerCase().indexOf(search) > -1)
    );
  }
  ...
  addExtTownship(data, stepId) {
    ...
    this.loaderService.display(true);
    const stepData = this.resDraftData.filter(x => x.stepId === stepId)[0];
    if (stepData.reasonItemId === this.ReservationReason.EXTENSIONOFTOWNSHIPS || stepData.reasonItemId === ReservationReason.RELAYOUT) {
      if (stepData.reservationDraftRequests.length >= 1) {
    ...
    if (stepData.reasonItemId === this.ReservationReason.CREATIONOFFARMS) {
      if (stepData.reservationDraftRequests.length >= 1) {
        this.snackbar.openSnackBar('You can add only one region for ' + stepData.requestReason, 'Warning');
        this.loaderService.display(false);
        return;
      }
    }
