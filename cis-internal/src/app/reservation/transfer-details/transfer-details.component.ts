...
export class TransferDetailsComponent implements OnInit {
  ...
  dataLength: number;
  dataSource: any;
  displayedColumns2: string[] = ['referenceNumber', 'designation', 'lpi', 'status', 'reason', 'issueDate', 'expiryDate', 'expiryInDays', 'name', 'provinceName'];
  dataLength2: number;
  dataSource2: any;
  ...
  ngOnInit() {
    ...
    if (this.readonly) {
      this.displayedColumns2 = ['referenceNumber', 'designation', 'lpi', 'status', 'reason', 'issueDate', 'expiryDate', 'expiryInDays', 'name', 'provinceName'];
      this.displayedColumns = ['referenceNumber', 'designation', 'lpi', 'status', 'reason', 'issueDate', 'expiryDate', 'expiryInDays', 'name', 'provinceName'];
    } else if (!this.readonly) {
      this.displayedColumns2 = ['referenceNumber', 'designation', 'lpi', 'status', 'reason', 'issueDate', 'expiryDate', 'expiryInDays', 'name', 'provinceName', 'action'];
      this.displayedColumns = ['referenceNumber', 'designation', 'lpi', 'status', 'reason', 'issueDate', 'expiryDate', 'expiryInDays', 'name', 'provinceName', 'action'];
    }
  }
  ...
  loadInitials() {
    ...
    forkJoin([
    ...
    ]).subscribe(([provinces, reservationTransfers, tDrafts]) => {
      this.provinces = provinces.data;
      this.transferedDrafts = tDrafts.data;
      this.provinceName = this.provinces.filter(x => x.provinceId === this.provinceId)[0].provinceName
      this.province = this.provinceName
      var arr = [];
      arr.push(this.province)
      this.onRequestPriority(arr);
      let outcomeIds = [];
      for (let j = 0; j < tDrafts.data.length; j++) {
        outcomeIds.push(tDrafts.data[j].outcomeId);
      }

      this.reservationData = reservationTransfers.data.filter(x => !outcomeIds.includes(x.outcomeId) && x.provinceName === this.provinceName);
      this.myReservationTransfer = reservationTransfers.data.filter(x => outcomeIds.includes(x.outcomeId) && x.provinceName === this.provinceName);
      ...
      this.filteredTasks = reservationTransfers.data.filter(x => !outcomeIds.includes(x.outcomeId) && x.provinceName === this.provinceName);
  ...
  refreshTable2() {
    this.dataSource2 = new MatTableDataSource(this.myReservationTransfer);
    this.dataSource2.paginator = this.paginator.toArray()[1];
    this.dataSource2.sort = this.sort.toArray()[1];
    this.dataLength2 = this.dataSource2.data.length || 0;
  ...
  ngAfterChange() {
    for (let j = 0; j < this.sortArr.length; j++) {
      this.sort[0].sort({ id: this.sortArr[j].colName, start: this.sortArr[j].direction, disableClear: true });
    }
  ...
  addToTransfer(row) {
    this.loaderService.display(true);
    this.restService.addDraftToTransfer(this.draftId, row.outcomeId).subscribe(response => {
      if (response.code === 50000) {
        this.loaderService.display(false);
        return;
      } else {
  ...
  Remove(element) {
    this.loaderService.display(true);
    let transferId = this.transferedDrafts.filter(x => x.outcomeId == element.outcomeId)[0].transferId;
    this.restService.deleteTransferDraft(transferId).subscribe(response => {
      this.loadInitials();
      this.loaderService.display(false);
    }, () => {
      this.loaderService.display(false);
