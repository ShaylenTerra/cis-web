...
import { ArchitectAddModalDialogComponent } from '../modals/architect-add-modal/architect-add-modal.dialog';

@Component({
    selector: 'app-architect-users',
    templateUrl: './architect-users.component.html',
    styleUrls: ['./architect-users.component.css'],
    providers: [
...
export class ArchitectUsersComponent implements OnInit {
    ...
    getPlsUsers() {
        ...
        this.restService.listPlsUsers('ARCHITECT', this.pageNo)]).subscribe(([statuses, payload]) => {
            const temp = [];
            ...
            error => {
                this.loaderService.display(false);
                if (error.message === 'No Architect users found.') {
                    this.snackbar.openSnackBar('No Architect users found.', 'Message');
                } else {
    ...
    addNewUser() {
        const dialogRef = this.dialog.open(ArchitectAddModalDialogComponent, {
            width: '70%',
            height: '80%',
            data: 'Architect'
        });
