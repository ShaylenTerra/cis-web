...
export class ExternalUsersComponent implements OnInit {
    ...
    download() {
        const data: any = this.data,
            ...
            fileName = `externalUsers-${todayDate}`;
        const options = {
            fieldSeparator: ',',
            filename: fileName,
            quoteStrings: '"',
    ...
    getUsers() {
        this.loaderService.display(true);
            this.restService.getUsersList(this.pageNo, 'EXTERNAL').subscribe((external) => {
            this.pageSize = external.headers.get('X-Total-Count');
            const temp = [];
            for (let i = 0; i < external.body.data.length; i++) {
                temp.push(external.body.data[i]);
            }
            ...
            const rolesArr = [];
            for (let j = 0; j < external.body.data.length; j++) {
                for (let k = 0; k < external.body.data[j].userRoles.length; k++) {
                    rolesArr.push(external.body.data[j].userRoles[k]);
                }
            }
